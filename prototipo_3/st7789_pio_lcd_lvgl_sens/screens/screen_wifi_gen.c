/**
 * @file screen_wifi_gen.c
 * @brief Tela de configuração Wi-Fi – sincronização de horário
 *
 * Nesta tela eu implemento a interface de configuração de Wi-Fi,
 * permitindo que o usuário informe SSID e senha através de
 * textareas com teclado virtual. Esta tela também é responsável
 * por garantir a limpeza correta dos recursos ao ser fechada.
 */

#include "screen_wifi_gen.h"
#include "screen_menu_gen.h"
#include "../components/buttons/button_back_gen.h"

#include "lvgl.h"

/*********************
 * STATIC VARIABLES
 *********************/

/* Campo de texto para SSID */
static lv_obj_t * ta_ssid = NULL;

/* Campo de texto para senha */
static lv_obj_t * ta_pass = NULL;

/* Teclado virtual (criado dinamicamente) */
static lv_obj_t * keyboard = NULL;

/*********************
 * STATIC FUNCTIONS
 *********************/

/**
 * @brief Callback de eventos do teclado virtual
 *
 * Este callback trata os eventos de finalização ou cancelamento
 * do teclado. Quando o usuário confirma ou cancela a digitação,
 * eu removo o teclado da interface e libero o ponteiro.
 */
static void keyboard_event_cb(lv_event_t * e)
{
    lv_event_code_t code = lv_event_get_code(e);

    if (code == LV_EVENT_READY || code == LV_EVENT_CANCEL)
    {
        if (keyboard)
        {
            lv_obj_del(keyboard);
            keyboard = NULL;
        }
    }
}

/**
 * @brief Callback de foco dos textareas
 *
 * Sempre que um campo de texto recebe foco, eu crio o teclado
 * virtual na layer superior e associo automaticamente o
 * textarea ativo ao teclado.
 */
static void textarea_event_cb(lv_event_t * e)
{
    /* Apenas trato evento de foco */
    if (lv_event_get_code(e) != LV_EVENT_FOCUSED)
        return;

    /* Evito criar múltiplos teclados */
    if (keyboard)
        return;

    /* Crio o teclado na layer superior para não prender à tela */
    keyboard = lv_keyboard_create(lv_layer_top());
    lv_obj_set_size(keyboard, 480, 160);
    lv_obj_align(keyboard, LV_ALIGN_BOTTOM_MID, 0, 0);

    /* Associo o textarea focado ao teclado */
    lv_keyboard_set_textarea(keyboard,
                             lv_event_get_target(e));

    /* Registro callback de eventos do teclado */
    lv_obj_add_event_cb(keyboard,
                        keyboard_event_cb,
                        LV_EVENT_ALL,
                        NULL);
}

/**
 * @brief Callback do botão Conectar
 *
 * Este callback é acionado quando o usuário pressiona o botão
 * de conexão. Aqui eu recupero os valores digitados nos campos
 * de SSID e senha. A lógica real de conexão Wi-Fi será integrada
 * posteriormente neste ponto.
 */
static void connect_button_event_cb(lv_event_t * e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED)
        return;

    /* Recupero os dados digitados */
    const char * ssid = lv_textarea_get_text(ta_ssid);
    const char * pass = lv_textarea_get_text(ta_pass);

    /* Aqui entra a lógica real de conexão Wi-Fi */
    (void)ssid;
    (void)pass;

    /* Fecho o teclado caso esteja aberto */
    if (keyboard)
    {
        lv_obj_del(keyboard);
        keyboard = NULL;
    }
}

/**
 * @brief Callback do botão Voltar
 *
 * Retorna para a tela de menu e garante que todos os recursos
 * locais (como teclado virtual) sejam corretamente liberados.
 */
static void back_button_event_cb(lv_event_t * e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED)
        return;

    /* Remove teclado, se existir */
    if (keyboard)
    {
        lv_obj_del(keyboard);
        keyboard = NULL;
    }

    /* Retorna para a tela de menu com animação */
    lv_scr_load_anim(screen_menu_create(),
                     LV_SCR_LOAD_ANIM_MOVE_RIGHT,
                     250,
                     0,
                     true);
}

/**
 * @brief Callback de exclusão da tela
 *
 * Este callback garante a limpeza total dos objetos e ponteiros
 * estáticos quando a tela Wi-Fi é destruída.
 */
static void screen_delete_event_cb(lv_event_t * e)
{
    LV_UNUSED(e);

    if (keyboard)
    {
        lv_obj_del(keyboard);
        keyboard = NULL;
    }

    ta_ssid = NULL;
    ta_pass = NULL;
}

/*********************
 * GLOBAL FUNCTIONS
 *********************/

/**
 * @brief Cria a tela de configuração Wi-Fi
 *
 * Esta função cria toda a interface gráfica da tela de Wi-Fi,
 * incluindo título, campos de entrada, teclado virtual e botões
 * de ação. A tela não utiliza cache, sendo criada sob demanda.
 *
 * @return Ponteiro para o objeto raiz da tela Wi-Fi
 */
lv_obj_t * screen_wifi_create(void)
{
    /* Crio o objeto raiz da tela */
    lv_obj_t * screen = lv_obj_create(NULL);
    lv_obj_set_size(screen, 480, 320);

    /* Fundo com tom escuro profissional */
    lv_obj_set_style_bg_color(screen,
                              lv_color_hex(0x101820),
                              0);

    /* Callback para limpeza ao destruir a tela */
    lv_obj_add_event_cb(screen,
                        screen_delete_event_cb,
                        LV_EVENT_DELETE,
                        NULL);

    /***************************
     * TÍTULO
     ***************************/
    lv_obj_t * title = lv_label_create(screen);
    lv_label_set_text(title, "Wi-Fi settings");
    lv_obj_set_style_text_color(title,
                                lv_color_hex(0xFFFFFF),
                                0);
    lv_obj_set_style_text_font(title,
                               &lv_font_montserrat_20,
                               0);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 14);

    /***************************
     * ESTILO DOS TEXTAREAS
     ***************************/
    /* Estilo único para manter padrão visual profissional */
    static lv_style_t style_ta;
    lv_style_init(&style_ta);
    lv_style_set_bg_color(&style_ta, lv_color_hex(0x1C2633));
    lv_style_set_border_color(&style_ta, lv_color_hex(0x3A4A5A));
    lv_style_set_border_width(&style_ta, 2);
    lv_style_set_radius(&style_ta, 6);
    lv_style_set_text_color(&style_ta, lv_color_hex(0xFFFFFF));
    lv_style_set_pad_left(&style_ta, 10);
    lv_style_set_pad_right(&style_ta, 10);

    /***************************
     * SSID
     ***************************/
    ta_ssid = lv_textarea_create(screen);
    lv_obj_add_style(ta_ssid, &style_ta, 0);
    lv_obj_set_size(ta_ssid, 360, 44);
    lv_textarea_set_placeholder_text(ta_ssid,
                                     "Nome da rede (SSID)");
    lv_obj_align(ta_ssid, LV_ALIGN_TOP_MID, 0, 70);
    lv_obj_add_event_cb(ta_ssid,
                        textarea_event_cb,
                        LV_EVENT_FOCUSED,
                        NULL);

    /***************************
     * SENHA
     ***************************/
    ta_pass = lv_textarea_create(screen);
    lv_obj_add_style(ta_pass, &style_ta, 0);
    lv_obj_set_size(ta_pass, 360, 44);
    lv_textarea_set_placeholder_text(ta_pass, "Senha");
    lv_textarea_set_password_mode(ta_pass, true);
    lv_obj_align(ta_pass, LV_ALIGN_TOP_MID, 0, 125);
    lv_obj_add_event_cb(ta_pass,
                        textarea_event_cb,
                        LV_EVENT_FOCUSED,
                        NULL);

    /***************************
     * BOTÃO CONECTAR
     ***************************/
    lv_obj_t * btn_conn = lv_btn_create(screen);
    lv_obj_set_size(btn_conn, 170, 46);
    lv_obj_align(btn_conn, LV_ALIGN_TOP_MID, 0, 185);
    lv_obj_add_event_cb(btn_conn,
                        connect_button_event_cb,
                        LV_EVENT_CLICKED,
                        NULL);

    lv_obj_t * lbl_conn = lv_label_create(btn_conn);
    lv_label_set_text(lbl_conn, "Conectar");
    lv_obj_center(lbl_conn);

    /***************************
     * BOTÃO VOLTAR
     ***************************/
    lv_obj_t * btn_back =
        button_back_create(screen,
                           back_button_event_cb);
    lv_obj_align(btn_back,
                 LV_ALIGN_BOTTOM_RIGHT,
                 -14,
                 -14);

    return screen;
}
